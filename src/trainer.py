...
def trainer(args):
    ...
    if args.logs:
        ...
        writer = SummaryWriter(
            params.output_dir + "/" + args.model_name + "/" + args.logs + "/logs"
        )

        image_saved_path = (
            params.output_dir + "/" + args.model_name + "/" + args.logs + "/images"
        )
        model_saved_path = (
            params.output_dir + "/" + args.model_name + "/" + args.logs + "/models"
        )
    ...
    train_dset_loaders = torch.utils.data.DataLoader(
        train_dsets, batch_size=params.batch_size, shuffle=True, num_workers=1
    )
